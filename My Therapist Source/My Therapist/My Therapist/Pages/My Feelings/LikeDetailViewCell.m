...
#define SHOULD_REMOVE_LABEL_LIKE_TAG 98
#define TEST 80
...
@implementation LikeDetailViewCell{
...
- (void)initBase{
    ...
    for (UILabel *label in self.subviews) {
        if (label.tag == SHOULD_REMOVE_LABEL_LIKE_TAG) {
            [label removeFromSuperview];
        }
    }
    UILabel *viewLabelHugBy = [[UILabel alloc]initWithFrame:CGRectMake(33, 5, 35, 16)];
    viewLabelHugBy.font = self.fontUsers;
    viewLabelHugBy.text = @"Liked by:";
    viewLabelHugBy.tag = SHOULD_REMOVE_LABEL_LIKE_TAG;
    ...
    int i=0;
    int maxTag = 0;
    for (NSDictionary *dicHug in self.arrayLikes) {
        ...
        if (fCurrentX <fCurrentPointXandLabel) {
            ...
            [btnHugUser addTarget:self action:@selector(didClickLikeUser:) forControlEvents:UIControlEventTouchUpInside];
            ...
            if (self.arrayLikes.count - i) {
                [buttonMore setTitle:[NSString stringWithFormat:@"and %ld more",self.arrayLikes.count - i] forState:UIControlStateNormal];
                buttonMore.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
                [buttonMore addTarget:self action:@selector(didClickMoreLikeUser:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_hugButton setImage:[UIImage imageNamed:@"thumb up.png"] forState:UIControlStateNormal];
...
-(void)didClickMoreLikeUser:(id)sender{
    if (_delegate &&[_delegate respondsToSelector:@selector(didTouchMoreLikeUserButton:)]) {
        [_delegate performSelector:@selector(didTouchMoreLikeUserButton:) withObject:sender];
    }
}

- (void)didClickLikeUser:(id)sender{
    if (_delegate &&[_delegate respondsToSelector:@selector(didTouchLikeUserButton:)]) {
        [_delegate performSelector:@selector(didTouchLikeUserButton:) withObject:sender];
    }
}
